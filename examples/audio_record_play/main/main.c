...
#define AUDIO_SAMPLE_RATE  AUDIO_HAL_08K_SAMPLES
/* i2s_Sample_Rate * T * sizeof(uint8_t)
16bits,Mono，Record_time = T/2(s) */
#define BUF_SIZE  (8000 * 10 * sizeof(uint8_t))  
...
void app_main(void)
{
    ...
    es8311_codec_set_voice_volume(90);
    bsp_i2s_init(I2S_NUM_0, 8000);   //config channel_format=mono

    /* Allocate audio buffer */
    audio_buffer = (uint8_t *) heap_caps_malloc(BUF_SIZE, MALLOC_CAP_INTERNAL); 
